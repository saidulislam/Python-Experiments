    ...
    def __next__(self):
        ...
        for n in range(self.start, self.stop):
            for x in range(2, n):
                ...
                # n is prime, because we've gone through the entire loop without having a non-prime situation
                # next time we need to start from n + 1, otherwise we will be trapped on n
                self.start = n + 1
                # return n for this round
                return n
        # this is what tells Python we've reached the end of the generator
        raise StopIteration()

next_num = PrimeGenerator(10)
print(next(next_num))
print(next(next_num))